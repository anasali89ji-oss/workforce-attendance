...
import {
  ...
  CheckCircle2, Clock, Zap, X, Check
} from 'lucide-react'
...
export default function KanbanPage() {
  ...
  const createBoard = async () => {
    if (!newBoardName.trim()) { toast.error('Board name required'); return }
    setCreatingBoard(true)
    try {
      const res = await fetch('/api/kanban', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'create_board', name: newBoardName.trim(), description: '' }),
      })
      if (!res.ok) { toast.error('Failed to create board'); return }
  ...
  if (!board) return (
    <div className="page anim-fade-up">
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: 18 }}>
        <div style={{ width: 76, height: 76, borderRadius: 22, background: 'linear-gradient(135deg, var(--brand-50), var(--brand-100))', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--brand-200)' }}>
          <Columns2 size={34} color="var(--brand-500)" strokeWidth={1.5} />
        </div>
        <div style={{ textAlign: 'center', maxWidth: 340 }}>
          <h2 style={{ fontSize: 20, fontWeight: 800, color: 'var(--text)', marginBottom: 8, letterSpacing: '-0.02em' }}>No boards yet</h2>
          <p style={{ color: 'var(--text-3)', fontSize: 13, lineHeight: 1.6 }}>Create your first Kanban board to start tracking tasks, sprints, or workflows.</p>
        </div>
        {!showNewBoard ? (
          <button onClick={() => setShowNewBoard(true)} className="btn btn-primary" style={{ gap: 8, padding: '10px 22px', fontSize: 14 }}>
            <Plus size={16} strokeWidth={2.5} /> Create Your First Board
          </button>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%', maxWidth: 380 }}>
            <input
              className="input" autoFocus
              placeholder="Board name (e.g. Sprint 1, Product Roadmap)"
              value={newBoardName} onChange={e => setNewBoardName(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && createBoard()}
            />
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={createBoard} disabled={creatingBoard} className="btn btn-primary" style={{ flex: 1, gap: 6 }}>
                {creatingBoard ? <span className="spinner spinner-sm" /> : <Check size={14} strokeWidth={2.5} />}
                Create Board
              </button>
              <button onClick={() => { setShowNewBoard(false); setNewBoardName('') }} className="btn btn-ghost">Cancel</button>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div style={{ padding: '28px 32px', height: 'calc(100vh - 60px)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexShrink: 0, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.02em', display: 'flex', alignItems: 'center', gap: 8 }}>
            <Columns2 size={20} color="var(--brand-500)" strokeWidth={2} />{board.name}
          </h1>
          <p style={{ color: 'var(--text-3)', fontSize: 13, marginTop: 3 }}>
            ...
            {boards.length > 1 && boards.map((b,i) => (
              <button key={b.id} onClick={() => setBoardIdx(i)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: i === boardIdx ? 'var(--brand-500)' : 'var(--text-3)', fontWeight: i === boardIdx ? 700 : 400, fontSize: 13, padding: '0 3px' }}>{b.name}</button>
            ))}
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: 'var(--text-3)' }}>
            <GripVertical size={13} strokeWidth={1.8} />Drag cards between columns
          </div>
      ...
      {addingCol && (
        <div style={{ marginBottom: 14, display: 'flex', gap: 8, flexShrink: 0 }}>
          <input className="input input-sm" autoFocus placeholder="Column name..."
        ...
        {board.columns.sort((a, b) => a.position - b.position).map(col => {
          ...
          return (
            <div key={col.id}
              ...
              style={{ width: 280, minWidth: 280, flexShrink: 0, display: 'flex', flexDirection: 'column', background: isOver ? 'var(--brand-50)' : 'var(--surface-2)', border: `2px solid ${isOver ? 'var(--brand-400)' : 'var(--border)'}`, borderRadius: 16, transition: 'all 0.15s', maxHeight: '100%' }}
            >
              <div style={{ padding: '12px 14px 10px', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 800, color: 'var(--text-3)', background: 'var(--surface)', border: '1px solid var(--border)', padding: '1px 7px', borderRadius: 99 }}>{col.cards.length}</span>
              </div>
              <div style={{ flex: 1, padding: '0 10px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8, minHeight: 80 }}>
                {col.cards.sort((a,b) => a.position - b.position).map(card => {
                  ...
                  return (
                    <div key={card.id} draggable onDragStart={() => setDragCard({ id: card.id, fromCol: col.id })} onDragEnd={() => setDragCard(null)}
                      style={{ background: 'var(--surface)', borderRadius: 10, padding: '10px 12px', boxShadow: 'var(--shadow-xs)', border: `1px solid var(--border)`, borderLeft: `3px solid ${pri.color}`, cursor: 'grab', position: 'relative', transition: 'all 0.15s' }}
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                        ...
                        {card.due_date && <span style={{ fontSize: 10, color: new Date(card.due_date) < new Date() ? '#ef4444' : 'var(--text-3)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 3 }}><Clock size={9} strokeWidth={2} />{new Date(card.due_date).toLocaleDateString('en-US',{month:'short',day:'numeric'})}</span>}
                        {(card.labels || []).map(l => <span key={l} style={{ padding: '1px 6px', background: 'var(--brand-50)', color: 'var(--brand-600)', borderRadius: 4, fontSize: 10, fontWeight: 600 }}>{l}</span>)}
                      </div>
                ...
                {addingCard === col.id ? (
                  <div>
                    <input autoFocus value={newTitle} onChange={e => setNewTitle(e.target.value)}
                      ...
                      placeholder="Card title..." className="input input-sm" style={{ marginBottom: 8 }} />
                    <div style={{ display: 'flex', gap: 4, marginBottom: 8 }}>
                      {Object.entries(PRI).map(([k, v]) => (
                        <button key={k} type="button" onClick={() => setNewPri(k)} style={{ flex: 1, padding: '4px 0', borderRadius: 6, border: `1.5px solid ${newPri === k ? v.color : 'var(--border)'}`, background: newPri === k ? v.bg : 'transparent', cursor: 'pointer', fontSize: 10, fontWeight: 700, color: newPri === k ? v.color : 'var(--text-3)', transition: 'all 0.15s' }}>{v.label}</button>
                      ))}
                    </div>
                  ...
                  <button onClick={() => { setAddingCard(col.id); setNewTitle('') }}
                    style={{ width: '100%', height: 34, background: 'none', border: '1.5px dashed var(--border)', cursor: 'pointer', borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, color: 'var(--text-3)', fontSize: 12, fontWeight: 600, transition: 'all 0.15s' }}
                  ...
                  ><Plus size={13} strokeWidth={2.5} />Add card</button>
                )}
              </div>
            </div>
          )
