...
import { CalendarOff, Plus, Check, X, Clock, AlertCircle, ChevronDown, FileText, User } from 'lucide-react'
...
export default function LeaveRequestsPage() {
  ...
  const apply = async () => {
    ...
    setSubmitting(true)
    try {
      const lt = leaveTypes.find(t => t.id === form.leave_type)
      const body = { ...form, leave_type: lt ? lt.name.toLowerCase() : form.leave_type }
      ...
      {showForm && (
        <div style={{ background: '#fff', borderRadius: 14, padding: 24, marginBottom: 20, border: '1px solid #E2E8F0', boxShadow: '0 4px 20px rgba(0,0,0,0.07)', animation: 'fadeUp 0.25s ease' }}>
          ...
          <div style={{ display:'flex', gap:10, marginTop:16 }}>
            <button onClick={apply} disabled={submitting} style={{ height:38, padding:'0 20px', background:'#2563EB', color:'#fff', border:'none', borderRadius:8, fontSize:13, fontWeight:600, cursor:submitting?'not-allowed':'pointer', opacity:submitting?.7:1, display:'flex', alignItems:'center', gap:6 }}>
      ...
      <div style={{ background:'#fff', borderRadius:14, border:'1px solid #E2E8F0', boxShadow:'0 1px 4px rgba(0,0,0,0.05)', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', borderBottom:'1px solid #F1F5F9' }}>
          {([['all', requests.length], ['pending', counts.pending], ['approved', counts.approved], ['rejected', counts.rejected]] as const).map(([t, count]) => (
            <button key={t} onClick={() => setTab(t as 'all'|'pending'|'approved'|'rejected')} style={{
                    ...
                    <div style={{ display:'flex', gap:12, flex:1, minWidth:0 }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontWeight:600, color:'#0F172A', fontSize:13, display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                          ...
                          <span style={{ padding:'2px 8px', borderRadius:5, fontSize:11, fontWeight:700, background:'#F1F5F9', color:'#475569', textTransform:'capitalize' }}>
                            {req.leave_type}
                          </span>
                        </div>
