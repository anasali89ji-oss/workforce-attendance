...
export async function POST(req: NextRequest) {
  ...
  if (action === 'clock_in') {
    ...
    if (existing) {
      const { data, error } = await supabaseAdmin.from('attendance_logs')
        .update({ punch_in_at: now, status: 'punched_in', is_late: isLate, timesheet_note: notes, updated_at: now })
      ...
      const { data, error } = await supabaseAdmin.from('attendance_logs').insert({
        ...
        punch_in_at: now, status: 'punched_in', is_late: isLate, timesheet_note: notes,
      }).select().single()
      if (error) return NextResponse.json({ error: error.message }, { status: 500 })
      return NextResponse.json({ data })
    }
  ...
  if (action === 'clock_out') {
    ...
    const { data, error } = await supabaseAdmin.from('attendance_logs')
      .update({ punch_out_at: now, status: 'punched_out', net_duration_minutes: netMinutes, overtime_minutes: overtimeMinutes, is_overtime: overtimeMinutes > 0, updated_at: now })
      .eq('id', existing.id).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ data })
  }
