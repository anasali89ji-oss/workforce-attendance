...
export async function POST(req: Request) {
  ...
  // Profile id must equal Supabase auth id or getCurrentUser() will find nothing.
  ...
  if (authError || !authData?.user) {
    ...
    return NextResponse.json({
      message: 'Supabase auth user creation failed — seeding aborted.',
  ...
  let result: { tenant: { id: string }; profile: { id: string } }
  try {
    result = await prisma.$transaction(async tx => {
      ...
      const profile = await tx.profile.create({
        data: {
          id: supabaseUserId, // CRITICAL: must match Supabase auth id
          tenant_id: tenant.id,
          email: adminEmail.toLowerCase(),
          password_hash: hash,
          full_name: 'Admin',
    ...
    // Roll back: delete Supabase user since DB transaction failed
    await supabaseAdmin.auth.admin.deleteUser(supabaseUserId).catch(() => {})
    console.error('[seed] DB transaction failed:', txError)
    return NextResponse.json({ message: 'Database transaction failed — seeding aborted.', error: String(txError) }, { status: 500 })
  }

  // Update Supabase user metadata with the actual tenant_id now we have it
  await supabaseAdmin.auth.admin.updateUserById(supabaseUserId, {
    user_metadata: { tenant_id: result.tenant.id, role: 'owner' },
  }).catch(e => console.error('[seed] Failed to update Supabase user metadata:', e))
